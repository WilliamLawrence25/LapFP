...
public class Ejercito{
  ...
  public Ejercito(String n) {
    ...
    for (int i = 0; i < random.nextInt(MAX_SOLDADOS-7)+1; i++) {
      Espadachin espadachin = new Espadachin(n, i);
      misSoldados.add(espadachin);}
    for (int j = 0; j < random.nextInt(MAX_SOLDADOS-8)+1; j++) {
      Arquero arquero=new Arquero(n, j);
      misSoldados.add(arquero);}
    for (int k = 0; k < random.nextInt(MAX_SOLDADOS-8)+1; k++) {
  ...
  public void mostrarSoldados() {
    ...
    for (Soldado s : this.misSoldados) {
      ...
      System.out.printf("%-12s(%s) | Vida:%d | Fila:%d | Columna:%d | Atq:%d | Def:%d\n", s.getNombre(), code,
